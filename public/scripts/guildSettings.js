...
const deleteGuild = () => {
  ...
  fetch(`/api/${APIVERSION}/guilds/${guild_id}`, {
    method: "delete",
    body: imageURL,
  }).then((response) => (window.location.href = "/app"));
};
...
const validateGuildName = () => {
  ...
  if (!isLongEnough) guildNameErrorMsg.innerHTML = "Guild name is too short";
    ...
    guildNameErrorMsg.innerHTML = "Invalid character(s)";
  else if (!isShortEnough)
    guildNameErrorMsg.innerHTML = "Guild name is too long";
...
const updateDefaultImage = () => {
  ...
  imageURL =
    `https://ui-avatars.com/api/?background=${getUniqueColor()}&name=` +
    guildName;
...
document
  .getElementById("deleteGuildBtn")
  .addEventListener("click", deleteGuild);
...
document
  .getElementById("updateGuildForm")
